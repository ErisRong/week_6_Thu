...
```{r}
old_lady_home_fall <- unintentional_injuries %>% 
...
falls_admissions_75_summary <- old_lady_home_fall %>% 
  summarise(NumberOfAdmissions = sum(NumberOfAdmissions, na.rm = TRUE))
```
...
NHS_healthboards_2 <- NHS_healthboards %>% 
  rename("HBR" = "HBCode")

joined_data <- NHS_healthboards_2 %>% 
  full_join(falls_admissions_75_summary, by = "HBR")
```
...
```{r}
plot(joined_data ["NumberOfAdmissions"]) 

ggplot(data= joined_data, aes(fill= NumberOfAdmissions)) +
  geom_sf() +
  scale_fill_distiller(palette = "Spectral")
  
#dark colour--more people fall in home
#lighter colour-less people falling
#darker colour= higher populationn density in darker coulour area
```
...
```{r}
specific_age_female_admissions <- hb2019_pop_est_14102024 %>% 
  filter(HB!= "S92000003",
         Sex == "Female",
         Year == "2022") %>% 
  select(-ends_with("QF")) %>%
  select(Year, HB, Sex, Age75:Age90plus) %>% 
  rename("HBR" = "HB") %>% 
  mutate(Age75plus = rowSums(across(Age75:Age90plus))) %>% 
  select(HBR, Age75plus)
  
joined_data_2 <- specific_age_female_admissions %>% 
   full_join(falls_admissions_75_summary, by = "HBR") %>% 
  mutate(FallsPer10K = (NumberOfAdmissions/Age75plus)*10000)

Joined_data_3 <- joined_data_2 %>% 
  full_join(NHS_healthboards_2, by = "HBR") %>% 
  st_as_sf()
  
  
ggplot(data= Joined_data_3, aes(fill= FallsPer10K)) +
  geom_sf() +
  scale_fill_distiller(palette = "Spectral")
```
